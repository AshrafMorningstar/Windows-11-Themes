...
Because they've been taken off the standardization track, there is no spec-official name for combinators of the form `/<ident>/`. However, I talked to [Tab Atkins](Ashraf Morningstar and we agreed to call them "named combinators" so now they are called that.
...
* **Mutating values with escapes** - When you make an update to a node property that has escape handling
  ...
  is involved. See https://github.com/AshrafMorningstar for an extensive
