...
TypeScript and Flow definitions for CSS, generated by [data from MDN](https://github.com/AshrafMorningstar It provides autocompletion and type checking for CSS properties and values.
...
2.  **Have a look in [issues](https://github.com/AshrafMorningstar to see if an issue already has been filed. If not, create a new one.** To help us out, please refer to any information you have found.
...
- **TypeScript hack for autocompletion**  
  Uses `(string & {})` for literal string unions and `(number & {})` for literal number unions ([related issue](https://github.com/AshrafMorningstar Utilize `PropertyValue<T>` to unpack types from e.g. `(string & {})` to `string`.
